...
export default function Jobs() {
    const jobJ = jobJson.filter(getRecentJob =>
        getRecentJob.type === 'job'
      );
      const listItems = jobJ.map(getRecentJob =>
        <Link to={getRecentJob.link}  key={getRecentJob.id}  className='grid grid-cols-1 mb-4 border-b border-zinc-100 pb-4'>
      ...
      return <div className='h-[712px] overflow-y-scroll pt-36 overflow-x-hidden mx-4'>{listItems}</div>;
}
