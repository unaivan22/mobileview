...
export default function Jobs() {
    const jobJ = jobJson.filter(getRecentJob =>
        ...
        <Link to={getRecentJob.link}  key={getRecentJob.id}  className='grid grid-cols-1 border-b border-zinc-100 mb-2 pb-2'>
            <div className='mx-4'>
                <div className='flex items-center gap-x-2'>
                    <img className='h-3 aspect-square object-cover rounded-xl col-span-1 my-1' src={getRecentJob.image} />
      ...
      return <div className='h-[530px] overflow-y-scroll overflow-x-hidden'>{listItems}</div>;
}
